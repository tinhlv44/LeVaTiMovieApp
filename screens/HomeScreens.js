...
import {  createShowtimesForNextWeek, getMoviesFromShowtimesToday, getShowtimesForNext7Days } from "../test";
const ios = Platform.OS == "android";
...
export default function HomeScreens() {
  ...
  const getUpcomingMovies = async () => {
    //const data = await fetchUpcomingMovies();
    const data = await getMoviesFromShowtimesToday();
      ...
      {loading ? (
        ...
        >
          ...
          <MovieList 
            ...
            data={upcoming} 
            onPress={() => {navigation.navigate("ListMovie", {title: "Sắp chiếu", data:upcoming})}}
          />
          ...
          <MovieList 
            ...
            data={topRated} 
            onPress={() => {navigation.navigate("ListMovie", {title: "Nổi bật", data:topRated})}}
          />
        </ScrollView>
      )}
    </View>
